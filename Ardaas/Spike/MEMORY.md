# On-device translation: memory pass (#35)
...
Latency had headroom (1662 ms/sentence); memory did not. This pass attacks
memory only — output is bit-identical.

**Result: 1374 MB → ~555 MB expected on device**, with no meaningful latency
regression. Session creation actually got ~4.8× *faster*.
...
Scaled by the host↔device ratio, **~555 MB expected on an iPhone 15 Pro**.

### 1. Sequential session lifecycle (−294 MB)

`BentiTranslator` now creates each session immediately before it is needed and
...
- **Disk.** The optimized cache is ~667 MB on top of the 352 MB of originals
  (~1.0 GB total). Dropping `save_external_prepacked_constant_initializers`
  halves the extra to ~467 MB at the cost of 65 MB of peak. The lab screen shows
  cache size and offers a delete button. Productizing should delete the
  originals once the cache is built, or ship the optimized form directly.
...
- **Host-measured.** Anonymous-memory-on-Linux is a proxy for iOS
  `phys_footprint`. The mechanism is confirmed at the XNU-ledger level, but the
  absolute numbers need the cable run.
...
3 GB minimum), rather than gating on RAM. At ~555 MB the pipeline fits inside
the tightest limit with roughly 2.5× headroom, where the 1374 MB baseline left
essentially none.

Two guards are still worth adding before this ships to users:

1. Check `os_proc_available_memory()` before starting a translation and refuse
   below ~800 MB rather than risking a jetsam kill.
2. Release sessions on `UIApplication.didReceiveMemoryWarningNotification`.
   Cheap now that sessions are already per-stage and reload in ~204 ms.

The 3 GB tier should be confirmed on real hardware (an iPhone SE 3 or 11) before
the policy is treated as settled — the 3 GB jetsam figure is inferred from
iPhone X measurements rather than directly measured.
...
On device: Benti Lab → pick **Baseline** or **Optimized** in the Pipeline
picker, Load model, Translate. The Memory section reports peak footprint for
that translation, the delta against the 1374 MB baseline, per-stage footprints,
and remaining jetsam headroom.
