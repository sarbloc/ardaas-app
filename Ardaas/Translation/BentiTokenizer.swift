...
// Source-side tokenizer (#35) implemented directly from `tokenizer_src.json`
// (HF *tokenizers* fast format), per spec section 3:
//
...
final class BentiTokenizer {
    ...
    /// Encoder positional limit. One benti is one model call — there is no
    /// sentence splitting — so anything longer has to be refused rather than
    /// silently truncated into a mistranslation.
    static let maxSourceTokens = 256

    /// Pure guard so the refusal is testable without the 359 MB artifact.
    static func validateSourceLength(_ ids: [Int64]) throws {
        guard ids.count <= maxSourceTokens else {
            throw TranslationError.inputTooLong(tokens: ids.count, limit: maxSourceTokens)
        }
    }
    ...
    init(tokenizerFile: URL) throws {
        func malformed(_ detail: String) -> TranslationError {
            .malformedModelFile(name: tokenizerFile.lastPathComponent, detail: detail)
        }
        ...
        for (rank, merge) in rawMerges.enumerated() {
            guard merge.count == 2 else {
                throw malformed("merge entry with \(merge.count) parts")
            }
        ...
        for entry in rawAdded {
            guard
                ...
                throw malformed("bad added_tokens entry")
            }
            added[content] = id
        }
