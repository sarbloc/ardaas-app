...
final class ArdaasContentTests: XCTestCase {
    ...
    func testBundledBuddhaDalContent() throws {
        ...
        // pins it to the engine's output. The English layer (#60) borrows the
        // SGPC translation wherever a passage coincides and renders the
        // divergent passages fresh, in the same register.
        XCTAssertTrue(content.hasTransliteration)
        XCTAssertTrue(content.hasEnglish)
        // hasTransliteration/hasEnglish read the first segment only;
        // validate() (run by loadBundled) is what guarantees all-or-none
        // coverage, so assert the full counts too rather than trusting one
        // segment.
        XCTAssertEqual(content.segments.filter { $0.transliteration != nil }.count, 16)
        XCTAssertEqual(content.segments.filter { $0.english != nil }.count, 16)
    ...
    func testBundledPlaceholdersAreDeclaredPerLayer() throws {
        ...
        // Since #60 Buddha Dal carries English too, and its placeholder
        // follows SGPC's: the parenthetical instruction is part of the marker
        // so that naming an occasion takes the instruction away with the dots.
        XCTAssertEqual(
            buddhaDal?.placeholder.english,
            "\(Self.slotMarker) (mention here the paath recited or the occasion)"
        )
    }
