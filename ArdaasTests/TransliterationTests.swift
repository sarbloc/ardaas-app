...
/// Spike (#35): postprocessing parity for the pure text stages —
...
final class TransliterationTests: XCTestCase {
    ...
    func testDandaIsPreserved() {
        XCTAssertEqual(
            BentiTranslator.transliterateDevanagariToGurmukhi("सतिगुरू जी ।"),
            "ਸਤਿਗੁਰੂ ਜੀ ।"
        )
    }
    ...
    func testDoubleDandaIsPreserved() {
        XCTAssertEqual(
            BentiTranslator.transliterateDevanagariToGurmukhi(
    ...
    func testAsciiAndPlaceholdersPassThrough() {
        XCTAssertEqual(
            BentiTranslator.transliterateDevanagariToGurmukhi("ID1 <ID1> abc 123 !"),
            "ID1 <ID1> abc 123 !"
        )
    }
    ...
    func testDevanagariDigitsMapIntoGurmukhiBlock() {
        XCTAssertEqual(
            BentiTranslator.transliterateDevanagariToGurmukhi("देवनागरी अंक ० १ ९"),
            "ਦੇਵਨਾਗਰੀ ਅਂਕ ੦ ੧ ੯"
        )
    }
    ...
    func testBlockBoundaries() {
        ...
        XCTAssertEqual(
            BentiTranslator.transliterateDevanagariToGurmukhi("ॐ ॰"),
            "\u{0A50} ॰"
        )
    }
    ...
    func testNonDevanagariScriptsPassThrough() {
        XCTAssertEqual(
            BentiTranslator.transliterateDevanagariToGurmukhi("বাংলা mixed वाह"),
            "বাংলা mixed ਵਾਹ"
        )
    }
    ...
    func testDandaAttachesLeft() {
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("ਸਤਿਗੁਰੂ ਜੀ ।"),
            "ਸਤਿਗੁਰੂ ਜੀ।"
        )
    }
    ...
    func testPunctuationAttachment() {
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("( ਸਤ ) ਅਤੇ [ ਨਾਮ ] ਜਪੋ !"),
            "(ਸਤ) ਅਤੇ [ਨਾਮ] ਜਪੋ!"
        )
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("ਏ - ਬੀ ਅਤੇ ਸੀ / ਡੀ"),
            "ਏ-ਬੀ ਅਤੇ ਸੀ/ਡੀ"
        )
    }
    ...
    func testQuotesAlternateAttachment() {
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("' ਸਤ ਸ੍ਰੀ ਅਕਾਲ ' ਆਖੋ ।"),
            "'ਸਤ ਸ੍ਰੀ ਅਕਾਲ' ਆਖੋ।"
        )
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("\" ਧੰਨ ਗੁਰੂ \" , ਉਸ ਨੇ ਕਿਹਾ ।"),
            "\"ਧੰਨ ਗੁਰੂ\", ਉਸ ਨੇ ਕਿਹਾ।"
        )
    }
    ...
    func testNumberSequencesCollapse() {
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("ਮਿਤੀ 15 . 8 . 2026 ਨੂੰ ਅਰਦਾਸ ਹੈ ।"),
            "ਮਿਤੀ 15.8.2026 ਨੂੰ ਅਰਦਾਸ ਹੈ।"
        )
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("ਸਮਾਂ 9 : 30 ਵਜੇ ॥"),
            "ਸਮਾਂ 9:30 ਵਜੇ॥"
        )
    }
    ...
    func testLeadingNumberSequenceQuirk() {
        XCTAssertEqual(
            BentiTranslator.indicDetokenize("10 , 000 ਰੁਪਏ"),
            "10, 000 ਰੁਪਏ"
        )
    }
    ...
    func testCleanupTokenizationSpaces() {
        XCTAssertEqual(
            BentiTranslator.cleanUpTokenizationSpaces("ਇਹ ਹੈ ਜੀ ."),
            "ਇਹ ਹੈ ਜੀ."
        )
        XCTAssertEqual(
            BentiTranslator.cleanUpTokenizationSpaces("ਕੀ ਹਾਲ ਹੈ ?"),
            "ਕੀ ਹਾਲ ਹੈ?"
        )
        XCTAssertEqual(
            BentiTranslator.cleanUpTokenizationSpaces("ਏ , ਬੀ"),
            "ਏ, ਬੀ"
        )
        // The quote rule: " x ' y " -> " x' y" per the reference replace order.
        XCTAssertEqual(
            BentiTranslator.cleanUpTokenizationSpaces("ਓ ' ਸ"),
            "ਓ'ਸ"
        )
    }
    ...
    func testPlaceholderRoundTrip() {
        ...
        for spelling in ["<ID1>", "< ID1 >", "[ID1]", "<id1>"] {
            XCTAssertEqual(
                BentiTranslator.restorePlaceholders("ਸਫਲਤਾ \(spelling) ਨੂੰ", placeholders: wrapped.placeholders),
    ...
    func testGurmukhiPurityGate() {
        XCTAssertTrue(BentiTranslator.isGurmukhiPure("ਵਾਹਿਗੁਰੂ ਜੀ ।"))
        XCTAssertTrue(BentiTranslator.isGurmukhiPure("\"ਧੰਨ\", ਹੈ ਜੀ।"))
        XCTAssertFalse(BentiTranslator.isGurmukhiPure("ਵਾਹਿਗੁਰੂ जी"))  // Devanagari leak
        XCTAssertFalse(BentiTranslator.isGurmukhiPure("ਜੀ latin"))
        XCTAssertFalse(BentiTranslator.isGurmukhiPure("ਜੀ 123"))       // ASCII digits are N
    }
}
